...
import { LayoutDashboard, MapPin, ShoppingCart, Package, Users, LogOut, Cloud } from 'lucide-react'
...
export function AppLayout() {
  ...
  // Onboarding state — kept as no-ops since OnboardingOverlay may not be in this worktree
  const [showOverlay, setShowOverlay] = useState(false)
  useEffect(() => { void setShowOverlay }, [])

  return (
    <div className={`min-h-screen bg-parchment ${isIndicatorVisible ? 'pt-8' : ''}`}>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 h-screen w-64 bg-sidebar text-white flex-col z-30">
        ...
        {/* Navigation */}
        <nav className="flex-1 py-5 overflow-y-auto" aria-label="Navegacion principal">
